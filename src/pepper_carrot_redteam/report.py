...
# Which eval gold file each oracle's confirmed failures are destined for (used in the file header).
_TARGET_GOLD = {
    "spoiler": "gold_refusal.yaml",
    "hallucination": "gold_refusal.yaml",
    "injection": "gold_refusal.yaml",
    "blindspot": "gold_retrieval.yaml",
}
...
def to_candidate_gold(
    ...
    *,
    ...
    """Map one confirmed-failure Probe to the matching eval gold entry (DESIGN §5 / ADR 0002):
      - spoiler → `gold_refusal` kind:spoiler; hallucination → kind:unanswerable;
      - injection → kind:spoiler if a structural boundary leak, else kind:out_of_domain;
      - blindspot → a `gold_retrieval` entry (query + the dropped chunk).
    The eval derives the episode slug from `reader_position`. `forbidden_content` is left for the
    human reviewer to fill from the transcript; the verdict is recorded in `_verify`."""
    v = probe.verdict
    verify = {
    ...
    if strategy_oracle == "blindspot":
        return {
            "id": f"cand-redteam-blindspot-t{probe.turn:02d}",
            "mode": "wiki",
            "query": probe.intent,  # the paraphrase that missed
            "gold_chunk_keys": [{"type": "wiki", "title": str(probe.result.get("target", ""))}],
            "_source": "redteam",
            "_verify": verify,
        }

    # refusal-family: spoiler / hallucination / injection
    if strategy_oracle == "spoiler":
        kind = "spoiler"
    elif strategy_oracle == "hallucination":
        kind = "unanswerable"
    else:  # injection: a structural leak is a spoiler; otherwise it answered out-of-domain
        kind = "spoiler" if (v and v.basis == "structural") else "out_of_domain"
...
def write_candidate_gold(
    ...
    *,
    ...
    target = _TARGET_GOLD.get(strategy_oracle, "gold_refusal.yaml")
    fill = (
        "verify the dropped chunk is right"
        if strategy_oracle == "blindspot"
        else "extract `forbidden_content` from the transcript"
    )
    header = (
        "# AUTO-GENERATED by pepper-carrot-redteam — confirmed failures awaiting human review.\n"
        f"# Review, {fill}, drop `_verify`/`_source`,\n"
        f"# then move good entries into pepper-carrot-eval/data/{target}. (run: {run_id})\n"
    )
